...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Botccoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no Botccoin: URI
    if(!uri.isValid() || uri.scheme() != QString("Botccoin"))
        return false;
...
{
    // Convert Botccoin:// to Botccoin:
    //
    //    Cannot handle this later, because Botccoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Botccoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Botccoin:");
    }
...
{
    QString ret = QString("Botccoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Botccoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Botccoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Botccoin (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Botccoin\n";
        else
            optionFile << strprintf("Name=Botccoin (%s)\n", chain);
