...
/* 1.14.5: set the wallet's discard threshold to 1 Botc because that's what 97%
...
/*
 * Botccoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Botccoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * Botccoin: Get a fee targetting a specific transaction speed.
     */
    CAmount GetBotccoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Botccoin: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetBotccoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Botccoin dust fee.
     */
