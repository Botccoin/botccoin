...
#if defined(NDEBUG)
# error "Botccoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Botccoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetBotccoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Botccoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("Botccoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Botccoin: BIP30 has been active since inception
    ...
    // Botccoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetBotccoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Botccoin: Disallow AuxPow blocks before it is activated.
    ...
    // Botccoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Botccoin: We probably want to disable this
    int nLockTimeFlags = 0;
